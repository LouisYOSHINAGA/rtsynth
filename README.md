...
| `-r, -b` | サンプルレート / バッファサイズ |
| `-g, --gain <0..1>` | **マスター音量**。音源の出力に対して最後に掛かり、プリセットの外側にあります（下記） |
...
| | どこ | プリセットとの関係 | 設定方法 |
|---|---|---|---|
| **マスター音量** | ホスト側。音源の出力に最後に掛かる | **プリセットの外**。切替でも revert でも変わらない | `-g 0.3` |
| 音源のゲイン | 音源の中（sine は `gain`、pd は `volume`） | **プリセットの一部**。切り替えると読み直される | `--param volume=0.3`、CC7、ツマミ |

**「全体的に音が大きい」は `-g` で下げてください。** 音源側のパラメータで下げると、
それは音色の一部なので**プリセットを切り替えた時点で元に戻ります**
（切替時に離脱するスロットへ保存され、次のスロットの値が読み込まれるため）。
...
- 編集内容は RAM 上のみで、**再起動するとファクトリの状態に戻ります**（保存は未実装）。
  ユーザスロットも例外ではありません
- マスター音量 (`-g`) は**プリセットに含まれません**。切り替えても revert しても
  変わらないので、機器全体の音量はこちらで決めてください（[1.3](#13-起動)）
- **プリセット切替時は、鳴っている音を先に消してから**新しい値を読み込みます。
  プリセットは波形やエンベロープも含めて全パラメータを一度に置き換えるため、
  そのまま切り替えるとリリース中の音に新しい音色が適用され、**勝手に鳴り直したように
  聞こえます**。数 ms かけて出力をフェードし、ボイスを落としてから読み込むので、
  クリックノイズも出ません（`revert` も同じ扱いです）
...
`external/pd` の `EG::update()` は、**途中のステップ**から次へ進む条件が
`dLevel_ > 0` / `dLevel_ < 0` の**厳密不等号**です。そのため EG の連続する 2 ステップの
レベルが等しく `dLevel_ == 0` になると、そのステップから永久に進めず、
**ボイスが解放されません**（`activeVoices` が減らないまま音が残る）。

エンド点でのこの症状（CC で DCA のエンド点を動かすと発生していたもの）は、
pd 側の EG 改修で**修正済み**です（エンド点の判定のみ `>=` / `<=` になりました）。
途中ステップの方は残っています。実測での確認:

| ケース | 離鍵 5 秒後の `activeVoices` |
|---|---|
| 既定パッチ | 0（正常） |
| DCA エンド点を移動（旧・再現手順） | 0（**修正済み**） |
| DCA のレベル 1 と 2 を同値にする（途中ステップ） | **1（音が残る）** |

`dLevel_ == 0 ||` を途中ステップ側の条件にも足せば解消しますが、`external/pd` は
**無改変で使う方針**のため rtsynth 側では修正していません。pd リポジトリ側の問題です。
