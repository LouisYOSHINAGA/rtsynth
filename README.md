...
| `StuckNoteGuard.hpp` | ノートオフを取りこぼす MIDI 機器への安全網。押鍵中のノートを記録し、一定時間オフが来なければ合成ノートオフを注入する（フレーム計数なので RT スレッドで時刻取得不要） | — |
...
- **MIDI の取りこぼし（音が鳴りっぱなし）の切り分け手順**:
  ...
  4. **`--midi-raw --midi-dump` で生バイト列を確認する**。`[bytes] 90 43 63` のように
     ワイヤ上のバイトがそのまま表示されるので、「鍵盤が本当にノートオフを送ったか」を
     推測せず確定できます。オフのバイト（`80 43 xx` または `90 43 00`）が**無い**なら
     損失は鍵盤／USB 側、**有る**のに発音が続くならシンセ側の問題です
  5. `[warning] MIDI device read errors` が出る場合、カーネルの rawmidi バッファが
     オーバーランしてバイトが失われています（この場合パーサを再同期します）
  6. 鍵盤側が原因のときは、別ケーブル・別 USB ポート・セルフパワー USB ハブ、
     および**鍵盤のファームウェア更新**を試してください
- **ノートオフ喪失時の安全網**: 上記で鍵盤側の問題と判明しても、シンセが永久に鳴り
  続けるのは実用上困るため、ホスト側で「一定時間ノートオフが来なかった音を自動的に
  離鍵する」機構（`core/StuckNoteGuard.hpp`）を持っています。既定 30 秒、
  `--note-timeout <秒>` で変更、`0` で無効。
  意図的なロングトーン（パッド・ドローン）を切らないよう既定値は長めですが、
  鍵盤の取りこぼしが多い場合は `--note-timeout 5` のように短くすると実用的です。
  発動時は `[warning] force-released a note whose note-off never arrived` が出るので、
  取りこぼしの頻度を把握できます（**本来の対処は鍵盤側の修正**であることに注意）
